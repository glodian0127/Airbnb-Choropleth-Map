...
def load_data(path):
    ...
    data_city = data_raw.loc[data_raw.city == cities[8]].reset_index(drop=True)
    return cities, data_city
...
def scale_data(data):
    data_model = data.drop("id", 1).drop("host_id", 1)  # drop ids' columns
    data_model = data.drop('State', 1).drop('city', 1).drop('host_listings_count', 1).drop('room_type', 1).drop('set', 1)
    # data_model['EmployTour/Employ'] = data_model['EmployTour']/data_model['Employ']
    # features = list(data_model.columns)
    data_model[COLS] = StandardScaler().fit_transform(data_model[COLS])
    data_scaled = data_model[COLS]
    return data_scaled
...
if __name__ == "__main__":
    cities_name, data = load_data("data_prepared.xlsx")
    # print(cities_name)
